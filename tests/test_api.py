...
def test_feeds_status(api_settings):
    response = api_settings.get("/api/feeds/status")
    assert response.status_code == 200
    assert "sources" in response.json()


def test_feeds_refresh(api_settings, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("src.feeds.cisa_kev.httpx.get", lambda *a, **k: FakeResponse())
    monkeypatch.setattr(
        "src.feeds.nvd_feed.request_with_backoff",
        lambda source, interval, fn, **kwargs: FakeResponse(),
    )
    monkeypatch.setattr("src.feeds.sources.httpx.get", lambda *a, **k: FakeResponse())

    from config import Settings

    settings = Settings()
    for module in ("config", "src.feeds.sources", "src.feeds.nvd_feed", "src.feeds.cisa_kev", "src.feeds.cache"):
        monkeypatch.setattr(f"{module}.settings", settings)

    response = api_settings.post("/api/feeds/refresh")
    assert response.status_code == 200
